...
const SHADERS_WORDMARK_SVG = `<svg width="1600" height="400" viewBox="0 0 1600 400" fill="none" xmlns="http://www.w3.org/2000/svg">
  <text x="800" y="180" text-anchor="middle" fill="#F4F4F0" font-family="'Helvetica Neue', Helvetica, Arial, sans-serif" font-size="110" font-weight="800" letter-spacing="-2">An investment in your</text>
  <text x="800" y="320" text-anchor="middle" fill="#F4F4F0" font-family="'Helvetica Neue', Helvetica, Arial, sans-serif" font-size="110" font-weight="800" letter-spacing="-2">career always pays back.</text>
</svg>`;
...
function transformEpiludeWordmarkSource(source: string, mode: EffectMode) {
  const palette =
    mode === "light"
      ? "[[184, 58, 20], [210, 90, 40], [140, 40, 12]]"
      : "[[184, 58, 20], [220, 100, 50], [255, 140, 80]]";

  return source
    .replace(
      "<title>Epilude — Footer</title>",
      "<title>Career Investment Particle Wordmark</title>",
    )
    .replace(
    ...
    .replace(
      ...
      `var WORDMARK = ${JSON.stringify(SHADERS_WORDMARK_SVG)};`,
    )
    .replace(
      "var PALETTE = [[255, 255, 255], [226, 232, 240], [191, 205, 225]];",
      `var PALETTE = ${palette};\nvar BG_TRANSPARENT = true;`,
    )
    .replace(
      "a: 0.04 + 0.95 * band * Math.pow(flake, 1.8)",
      "a: 0.25 + 0.75 * band * Math.pow(flake, 1.2)",
    )
    .replace(
      /--olive-950: #0c0c0d;/g,
      `--olive-950: ${mode === "dark" ? "#161618" : "#f8f9fb"};`,
    );
}
...
function NeuformIsolatedEffect({
  ...
  return (
    <iframe
      ...
      loading="eager"
      // @ts-expect-error allowTransparency is a legacy but functional iframe attribute
