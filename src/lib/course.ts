// Legacy course IDs for backward compatibility with existing payment system
...
export const OMNIROUTE_TITLE = "OmniRoute Setup for Free Claude Tokens";
export const OMNIROUTE_SUBTITLE =
...
// Dynamic course system integration
// These functions will be used to fetch courses from Firestore for the frontend
export const LEGACY_COURSE_IDS = [COURSE_ID, INTERNSHIP_ID, OMNIROUTE_ID, TESTING_ID] as const;

// Check if a course ID is from the legacy system
export function isLegacyCourseId(courseId: string): courseId is typeof LEGACY_COURSE_IDS[number] {
  return LEGACY_COURSE_IDS.includes(courseId as any);
}

// Get course price for legacy courses (for payment system)
export function getLegacyCoursePrice(courseId: string): number {
  switch (courseId) {
    case COURSE_ID:
      return COURSE_PRICE_INR;
    case INTERNSHIP_ID:
      return INTERNSHIP_PRICE_INR;
    case OMNIROUTE_ID:
      return OMNIROUTE_PRICE_INR;
    case TESTING_ID:
      return TESTING_PRICE_INR;
    default:
      return 0;
  }
}

// Get course title for legacy courses
export function getLegacyCourseTitle(courseId: string): string {
  switch (courseId) {
    case COURSE_ID:
      return COURSE_TITLE;
    case INTERNSHIP_ID:
    ...
    case OMNIROUTE_ID:
      return OMNIROUTE_TITLE;
    case TESTING_ID:
      return TESTING_TITLE;
    default:
